...
using DG.Tweening;
public class aUserCharacterMove : MonoBehaviour
{
    //캐릭터 직선 이동 속도 (걷기)
    ...
    void Update()
    {
        ...
        //중력 적용
        setGravity();
        UIManager.Instance.SetTexst(GameManager.instance.mycombo.ToString());
        UIManager.Instance.SetTexsts(GameManager.instance.damge.ToString());
    }
        ...
        if (flagNextAttack == true)
        {
            ...
            switch (playerAttackState)
            {
                ...
                case PlayerAttackState.atkStep_1:
                    ...
                    GameManager.instance.mycombo += 20;
                    Debug.Log(playerAttackState);
                    break;
                case PlayerAttackState.atkStep_2:
                ...
                case PlayerAttackState.atkStep_3:
                    ...
                    GameManager.instance.mycombo += 40;
                    break;
                case PlayerAttackState.atkStep_4:
                    playerAttackState = PlayerAttackState.atkStep_1;
    ...
    {
        bool comss = false;
        float dashRange = 8f; //대시 거리
        float dashTime = 4f; //대시 시간
        if (Input.GetKeyDown(KeyCode.R))
        {
            ...
            if (comss)
            {
                if (GameManager.instance.mycombo > 200)
                {
                    Debug.Log("afd");
                    GameManager.instance.mycombo -= 200;

                    transform.DOMove(transform.position + transform.up * dashRange, dashTime);
